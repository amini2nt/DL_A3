...
def calculate_fid_score(sample_feature_iterator,
    ...
    """
    p = np.asarray([i for i in sample_feature_iterator])
    mu_p = np.mean(p, axis=0)
    cov_p = np.cov(p.T)
    
    q = np.asarray([i for i in testset_feature_iterator])
    mu_q = np.mean(q, axis=0)
    cov_q = np.cov(q.T)
    
    return np.power(np.linalg.norm(mu_p - mu_q), 2) + np.trace(cov_p + cov_q - 2*np.power(cov_p*cov_q, 0.5))
