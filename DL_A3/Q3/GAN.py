...
def make_generator():
    ...
    model.add(Conv2DTranspose(3, (5, 5), padding='same', activation='tanh'))
    return model
...
def make_discriminator():
    ...
    model = Sequential()
    if K.image_data_format() == 'channels_first':
        model.add(Convolution2D(64, (5, 5), padding='same', input_shape=(3, 32, 32)))
    else:
        model.add(Convolution2D(64, (5, 5), padding='same', input_shape=(32, 32, 3)))
    model.add(LeakyReLU())
...
for epoch in range(600):
    ...
    for i in range(int(X_train.shape[0] // (BATCH_SIZE * TRAINING_RATIO))):
        ...
        generator_loss = generator_model.train_on_batch(np.random.rand(BATCH_SIZE,
                                                                            100),
                                                             positive_y)
        
    msg = "Discriminator loss: " + str(discriminator_loss)
    msg = "%s Generator loss: %f" % (msg, generator_loss)
    print("\r" + msg)
